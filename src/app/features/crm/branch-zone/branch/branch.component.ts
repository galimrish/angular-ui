...
export class BranchComponent implements OnDestroy {
  ...
  displayedColumns: string[] = ['name', 'address', 'id', 'Operations'];
  ...
  openEditDialog(branch?: Branch): void {
    const dlgRef = this.dialog.open(BranchEditComponent, {
      width: '500px',
      data: { type: branch || new Branch() }
    });
    ...
    this.subscriptions.push(
      this.isLoading$.pipe(
        ...
        switchMap(() => dlgRef.afterClosed())
          ...
          if (result !== undefined) {
            if (branch) {
              this.store.dispatch(BranchActions.update({
                update: {id: branch.id || '', changes: result}
              }));
            } else {
              this.store.dispatch(BranchActions.create({
                branch: new Branch(undefined, result.name, result.code)
              }));
            }
            this.search();
          }
